...
class FavouritesViewModelTest {
    ...
    @Test
    fun loadFavouritesWhenErrorReturnsError() = runTest {
        coEvery { repository.getFavoriteFilms() } returns flow {
        ...
        val state = viewModel.uiState.value
        assertTrue(state is FavouritesUiState.Error)
    }

    @Test
    fun removeFromFavouritesCallsRepositoryAndReloads() = runTest {
        coEvery { repository.getFavoriteFilms() } returns flowOf(favouriteFilms)
